...
def save_checkpoint(cfg, model_in, epoch, optimizer=None, scheduler=None,
                    additioanl_dict=None, is_best=False, post_fix='ckpt',
                    save_name=None, is_logging=True, model_name=None):
    """
    model_in (nn.Module): model to save / (tuple): (model0, model1, ...) 
    model_name (str0, str1, ...): when more than one model to save, enter the stored model names
    """
    if save_name is None:
        save_name = cfg.run_name

    current_ckpt_name = f'{save_name}_{post_fix}_latest.pth'
    ...
    if type(model_in) != type(()):
        save_dict.update({'model': model_in.module.state_dict() if hasattr(model_in, 'module') else model_in.state_dict()})
    else:
        for i, model in enumerate(model_in):
            save_dict.update({model_name[i]: model.module.state_dict() if hasattr(model, 'module') else model.state_dict()})
    ...
    if cfg.save_freq > 0 and epoch % cfg.save_freq == 0:
        milestone_ckpt_name = f'{save_name}_{post_fix}_E{epoch}.pth'
    ...
    if is_best:
        best_ckpt_name = f'{save_name}_{post_fix}_best.pth' if save_name else f'{post_fix}_best.pth'
...
def save_prior(cfg, memory_prior, epoch, is_best=False, post_fix='prior_latest', is_logging=True):
    save_name = cfg.run_name
    current_prior_name = f'{save_name}_{post_fix}.pth'
    current_prior_path = os.path.join(cfg.ckpt_dir, current_prior_name)

    torch.save(memory_prior, current_prior_path)

    if cfg.save_freq > 0 and epoch % cfg.save_freq == 0:
        milestone_prior_name = f'{save_name}_prior_E{epoch}.pth'
        milestone_prior_path = os.path.join(cfg.ckpt_dir, milestone_prior_name)
        shutil.copyfile(current_prior_path, milestone_prior_path)
        logging.info("Saved in {}".format(milestone_prior_path))

    if is_best:
        best_prior_name = f'{save_name}_prior_best.pth' if save_name else 'prior_best.pth'
        best_prior_path = os.path.join(cfg.ckpt_dir, best_prior_name)
        shutil.copyfile(current_prior_path, best_prior_path)
        if is_logging:
            logging.info("Found the best prior information for main model and saved in {}".format(best_prior_path))


def resume_checkpoint(config, model_in, optimizer=None, scheduler=None, pretrained_path=None, printer=logging.info, model_name=None):
    """
    model_in (nn.Module): model to resume / (tuple): (model0, model1, ...) 
    model_name (str0, str1, ...): when more than one model to save, enter the stored model names
    """
    ...
    if type(model_in) != type(()):
        ...
        for i, model in enumerate(model_in):
            ckpt_state = checkpoint[model_name[i]]
            model_dict = model.state_dict()
            # rename ckpt (avoid name is not same because of multi-gpus)
            is_model_multi_gpus = True if list(model_dict)[0].split('.')[0] == 'module' else False
            is_ckpt_multi_gpus = True if list(ckpt_state)[0].split('.')[0] == 'module' else False
...
def load_checkpoint(model_in, pretrained_path, model_name=None):
    """
    model_in (nn.Module): model to load / (tuple): (model0, model1, ...) 
    model_name (str0, str1, ...): when more than one model to save, enter the stored model names
    """
    if not os.path.exists(pretrained_path):
    ...
    if type(model_in) != type(()):
        ...
        for i, model in enumerate(model_in):
            ckpt_state_dict = state_dict[model_name[i]]
            base_ckpt = {k.replace("module.", ""): v  for k, v in ckpt_state_dict.items()}
        if hasattr(model, 'module'):
            incompatible = model.module.load_state_dict(base_ckpt, strict=False)
        else:
...
def load_prior(prior_path):
    if not os.path.exists(prior_path):
        raise NotImplementedError('no checkpoint file from path %s...' % prior_path)
    memory_prior = torch.load(prior_path)
    logging.info(f'Successful Loading the prior vector from {prior_path}')
    return memory_prior
