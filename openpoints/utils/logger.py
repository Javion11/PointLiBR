...
def generate_exp_directory(cfg,
    ...
    # cfg.code_dir = os.path.join(cfg.run_dir, 'code') # comment out the code dir, it's unnecessary to copy openpoints code to log/code directory
    cfg.log_path = os.path.join(cfg.run_dir, cfg.run_name + '.log')

    if cfg.get('rank', 0) == 0 and (not cfg.debug):
